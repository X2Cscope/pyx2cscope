...
class SerialStub:
    ...
    def LNetSerial_read16(self) -> bytearray:
        ...
        if self.data == self.get_device_info:
            return bytearray(
                b"U.\x01\x00\x00\x05\x00\x01\x00\xff\x10\x82Nov2320231500\x00\x00\x00\x00\x00\x00\xccJ"
                b"\x14K\xc0\xff\xff\x01\x00\x00\x00\x00\x00\x00\xf4R\x00\x00\xba"
            )
        elif self.data == self.load_param:
            return bytearray(
                b"U\x1f\x01\x11\x00\x00\x04\x00\x00\x98:\x00\x004\x10\x00\x00\xe0\x05\x00\x00\x00\x00\x00\x00\x98:\x00\x00\x98:\x00\x00\x82\xab"
            )
    ...
    def LNetSerial_read32(self) -> bytearray:
        ...
        if self.data == self.get_device_info:
            return bytearray(
                b"U.\x01\x00\x00\x05\x00\x01\x00\xff \x82Mar 320191220Mar 320191220\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00 T"
            )
        elif self.data == self.load_param:
            return bytearray(
                b"U\x1f\x01\x11\x00\x00\x00\x00\x00\x00\x00\x00\x00\xa8\x00\x00 \x00\x00\x00\x00\x00\x00\x00\x00\x00 \x00\x00\x00 \x00\x00\x82\x10"
            )
        assert False, "Unexpected message received"
