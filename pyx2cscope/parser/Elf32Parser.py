...
class Elf32Parser(ElfParser):
    ...
    def get_var_list(self) -> List[str]:
        ...
        for compilation_unit in self.dwarf_info.iter_CUs():
            ...
            tag_variables = list(filter(lambda die:
                                        die.tag == 'DW_TAG_variable' and
                                        'DW_AT_location' in die.attributes and
                                        isinstance(die.attributes["DW_AT_location"].value, List) and
                                        len(die.attributes["DW_AT_location"].value) > 2,
                                        root_die))
            die_variables = list(filter(lambda die: die.attributes.get('DW_AT_name') is not None, tag_variables))
            for die_variable in die_variables:
                ...
                if end_die.tag == "DW_TAG_structure_type":
                    ...
                    for member in members.keys():
                        my_list.append(die_variable.attributes["DW_AT_name"].value.decode("utf-8") + "." + member)
                else:
                    my_list.append(die_variable.attributes["DW_AT_name"].value.decode("utf-8"))
        my_list.insert(0,"")
    ...
    def get_var_info(self, var_name) -> VariableInfo:
        ...
        for compilation_unit in self.dwarf_info.iter_CUs():
            ...
            die_variables = list(
                filter(lambda die:
                       die.attributes.get('DW_AT_name') is not None and
                       die.attributes.get('DW_AT_name').value.decode('utf-8') == variable_name, tag_variables)
            )
            if die_variables:
                assert len(die_variables) == 1, 'Multiple variables found'
                die_variable = die_variables[0]
                ref_addr = die_variable.attributes.get('DW_AT_type').value + die_variable.cu.cu_offset
                ...
                if end_die.tag == "DW_TAG_pointer_type":
                    ...
                    return VariableInfo(
                        name=die_variable.attributes['DW_AT_name'].value.decode("utf-8") + "." + member_name,
    ...
    def get_end_die(self, current_die):
        ...
        """
        valid_words = ["DW_TAG_base_type", "DW_TAG_pointer_type", "DW_TAG_structure_type"]
