...
com_port = "COM32"
x2cScope = X2CScope(port=com_port)
...
angle_reference = x2cScope.get_variable("mcFocI_ModuleData_gds.dOutput.elecAngle")
speed_measured = x2cScope.get_variable("mcFocI_ModuleData_gds.dOutput.elecSpeed")
...
x2cScope.export_variables(filename="my_two_variables", items=[angle_reference, speed_measured])
# you can export all the variables available. For a different file format, define 'ext' variable, i.e. pickle (binary)
x2cScope.export_variables(filename="my_variables", ext=FileType.PICKLE)
